...
export const BottomNav: React.FC<BottomNavProps> = ({ activeSection, setSection }) => {
  const navItems: { id: Section; label: string; icon: string }[] = [
    ...
    { id: 'summaries', label: 'Summary', icon: '📊' },
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {isActive && (
                <div className="absolute top-2 w-12 h-8 bg-primary/10 dark:bg-primary/20 rounded-2xl animate-scale-in" />
              )}
              
              <span className={`text-xl mb-0.5 relative z-10 transition-transform duration-200 group-active:scale-90 ${
