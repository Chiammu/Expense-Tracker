        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              <span className={`text-lg mb-0.5 relative z-10 transition-transform duration-200 group-active:scale-90 ${isActive ? 'scale-110 -translate-y-0.5' : 'grayscale opacity-70'
                }`}>
                {item.icon}
              </span>

              <span className={`text-[9px] font-bold tracking-wide transition-all duration-200 ${isActive ? 'text-primary translate-y-0 opacity-100' : 'text-text-light translate-y-1 opacity-70'
                }`}>
                {item.label}
              </span>
            </button>
